<template>
  <div class="login-panel-account">
    <el-form
      ...
      label-width="60px"
    >
      <el-form-item label="账户" label-position="right" prop="username">
        <el-input v-model="accountInfo.username" :show-password="true" />
      </el-form-item>

      <el-form-item label="密码" label-position="right" prop="password">
        <el-input v-model="accountInfo.password" :show-password="true" />
      </el-form-item>
    </el-form>
  </div>
</template>
...
function LoginHandleAccountAction(isRemPWD: boolean): void {
  ...
  if (accountInfo.password !== '' && accountInfo.username !== '') {
    if (
      accountInfo.password === loginConfigCode.password &&
