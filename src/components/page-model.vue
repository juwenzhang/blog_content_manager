<template>
  <div class="system-user-model">
    <el-dialog v-model="dialogVisible"
               :title="props.header.addTitle || props.header.editTitle || '新建用户'" width="30%" center>
          ...
          <template v-for="(item, index) in props.propsList" :key="index">
            <el-form-item :label="item.label" :prop="item.prop">
              ...
              <template v-else-if="item.type === 'date'">
                <el-date-picker v-model="formData[item.prop]" type="daterange" :placeholder="item.placeholder"/>
              </template>
